...
fn multiple_receivers() {
  ...
  std::thread::spawn(move || {
    block_on(
      client
        .connection
        .send(ZeroMessage::request("ping", 0, RequestType::Ping(Ping()))),
    )
    .unwrap();
    block_on(
      client
        .connection
        .send(ZeroMessage::request("ping", 1, RequestType::Ping(Ping()))),
    )
    .unwrap();
    block_on(
      client
        .connection
        .send(ZeroMessage::request("ping", 2, RequestType::Ping(Ping()))),
    )
    .unwrap();
    block_on(
      client
        .connection
        .send(ZeroMessage::request("ping", 3, RequestType::Ping(Ping()))),
    )
    .unwrap();
  });
  std::thread::spawn(move || {
